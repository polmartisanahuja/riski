...
echo "#!/bin/bash
filebot -script fn:amc --output \"/media/MEDIABOX/media/\" --log-file amc.log --action copy --conflict override -non-strict --def music=y clean=y subtitles=en,es xbmc="$address" artwork=y \"ut_dir=\$TR_TORRENT_DIR/\$TR_TORRENT_NAME\" \"ut_kind=multi\" \"ut_title=\$TR_TORRENT_NAME\" \"exec=chmod 777 -R '/media/MEDIABOX'\"" | sudo tee /home/pi/transmission-postprocess.sh
